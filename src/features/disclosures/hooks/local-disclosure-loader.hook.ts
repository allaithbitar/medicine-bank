...
export async function getLocalDisclosure(id: string) {
  const res = (await localDb
      ...
      jsonObjectFrom(
        col
          ...
          .select((pcol) => [
            ...
            jsonObjectFrom(
              pcol.selectFrom('areas').select(['areas.id', 'areas.name']).whereRef('areas.id', '=', 'patients.areaId')
            ).as('area'),
          ])
    ...
    .executeTakeFirst()) as unknown as TDisclosure | null;
