...
export const getLocalDisclosureDetails = async (disclosureId: string) => {
  const result = (await localDb
    .selectFrom('disclosure_properties')
    .selectAll()
    .select((eb) => [withCreatedBy(eb, 'createdBy'), withUpdatedBy(eb, 'updatedBy')])
...
export const useLocalDisclosureDetailsLoader = (disclosureId?: string, forceOffline = false) => {
  ...
  const queryResult = useQuery({
    ...
    queryFn: async () => getLocalDisclosureDetails(disclosureId || ''),
