...
const DisclosureConsultingAdviserCards = ({
  disclosureId,
}: {
  disclosureId?: string;
}) => {
  ...
  const handleEditClick = useCallback(
    (adviserConsultation: TDisclosureAdviserConsultation) => {
      const consultingNoteCreatorId =
        (adviserConsultation.createdBy as any)?.id ??
        adviserConsultation.createdBy;
      const hasAccess = consultingNoteCreatorId === currentUserId;
      if (hasAccess) {
        navigate(
          `/disclosures/${adviserConsultation.disclosureId}/consulting_adviser/action`,
          {
