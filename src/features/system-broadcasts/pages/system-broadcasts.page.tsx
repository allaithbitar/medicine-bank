...
const SystemBroadcastsPage = () => {
  ...
  const [audienceFilter, setAudienceFilter] =
    useState<TBroadcastAudience>("all");

  const {
    data: { items: searchResp = [] } = { items: [] },
  ...
  const handleOpenModal = (oldBroadcast?: TSystemBroadcast) => {
    navigate("/system-broadcast/action", { state: { oldBroadcast } });
  };
      ...
      <Stack gap={3} sx={{ height: "100%" }}>
        ...
        <Box sx={{ px: 2 }}>
          <ToggleButtonGroup
            ...
            exclusive
            onChange={(_, v) => v && handleAudienceQuick(v)}
          >
            {AUDIENCES.map((a) => (
            ...
            >
              {({ item }) => (
                <SystemBroadcastCard
                  ...
                  onEdit={(b) => handleOpenModal(b as TSystemBroadcast)}
                />
              )}
            </VirtualizedList>
          )}
