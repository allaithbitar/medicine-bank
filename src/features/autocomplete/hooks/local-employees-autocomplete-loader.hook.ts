...
export const useLocalEmployeesAutocompleteLoader = (dto: TAutocompleteDto & { role?: string[] }) => {
  const isOffline = useIsOffline();
  const queryResult = useQuery({
    queryKey: ['LOCAL_EMPLOYEES_AUTOCOMPLETE', dto],
    queryFn: async () => {
      let baseQuery = localDb.selectFrom('employees').select(['id', 'name']);
      ...
      const items = (await query.execute()) as TAutocompleteItem[];
