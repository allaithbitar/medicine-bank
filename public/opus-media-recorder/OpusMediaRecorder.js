...
class OpusMediaRecorder extends EventTarget {
  ...
  constructor (stream, options = {}, workerOptions = {}) {
    ...
    // optional overrides for sample rate and channel count
    this._preferredSampleRate = preferredSampleRate || null;
    this._preferredChannelCount = typeof preferredChannelCount === 'number' ? preferredChannelCount : null;
  ...
  start (timeslice = Number.MAX_SAFE_INTEGER) {
    ...
    // Create audio context; allow preferredSampleRate override if provided
    this.context = this._preferredSampleRate ? new AudioContext({ sampleRate: this._preferredSampleRate }) : new AudioContext();
    let tracks = this.stream.getAudioTracks();
    if (!tracks[0]) {
      throw new Error('DOMException: UnkownError, media track not found.');
    }
    // channelCount: prefer explicit override, otherwise use track settings or default to 1
    const trackChannelCount = tracks[0].getSettings().channelCount;
    this.channelCount = this._preferredChannelCount || trackChannelCount || 1;
    // sampleRate: prefer AudioContext's sampleRate (which may reflect preferredSampleRate)
